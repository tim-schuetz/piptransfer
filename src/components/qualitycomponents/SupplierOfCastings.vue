...
import { fetchsupplierofcastings } from '@/services/qualitydataapis'
...
const title = ref('PP part - Casting')
...
async function loadchart(){
    ...
    try{
        const supplierofcastings = await fetchsupplierofcastings()
        const xdata_raw = supplierofcastings.map(element => element.supplier)
        xdata.value = xdata_raw.map(item => {
            return renameoptions.getsuppliershortname(item)
        })
        ydata.value = supplierofcastings.map(element => element.amount)
        initchart()
    } catch(error){
        console.error(error)
    } finally{
...
onMounted( async () =>{
    title.value = renameoptions.getcomponentrenaming('pppartcasting')
    loadchart()
})
